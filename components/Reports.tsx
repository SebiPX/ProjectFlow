    ...
    if (isError) {
        return (
            <div className="text-center py-20">
                ...
                <h3 className="text-xl font-bold text-foreground">Error loading reports</h3>
                <p className="text-muted-foreground mt-2">Could not fetch profitability data.</p>
            </div>
        );
    }
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto space-y-8">
            <div>
                ...
                <p className="mt-2 text-muted-foreground">Deep dive into service performance and profitability.</p>
            </div>
            ...
            <div className="bg-card p-6 rounded-xl border border-border shadow-lg">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-bold text-foreground flex items-center">
                        <Icon path="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" className="w-5 h-5 mr-2 text-primary" />
                        Service Profitability Overview
                    </h2>
                    <select
                        value={sortBy}
                        onChange={(e) => setSortBy(e.target.value as any)}
                        className="bg-muted text-foreground text-sm rounded-lg px-3 py-2 border border-input focus:outline-none focus:ring-2 focus:ring-primary"
                    >
            ...
            <div className="bg-card rounded-xl border border-border shadow-lg overflow-hidden">
                <div className="p-6 border-b border-border">
                    <h2 className="text-xl font-bold text-foreground">Performance Details</h2>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-background/50 text-muted-foreground text-sm uppercase">
                            <tr>
                        ...
                        <tbody className="divide-y divide-gray-700">
                            {sortedStats.map((stat) => (
                                <tr key={stat.service_module_id} className="hover:bg-muted/50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="font-medium text-foreground">{stat.service_name}</div>
                                    </td>
                                    <td className="px-6 py-4">
...
function getMarginColor(margin: number) {
    ...
    if (margin >= 20) return 'text-primary';  // Good
...
function getMarginBgColor(margin: number) {
    ...
    if (margin >= 20) return 'bg-primary';
